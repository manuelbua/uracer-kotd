package com.bitfire.uracer.game.world.models.loaders;
...
public class UG3dtLoader {
	...
	private static StillSubMesh readStillSubMesh( BufferedReader in, boolean flipV ) throws IOException {
		...
		Mesh mesh = new Mesh( VertexDataType.VertexArray, true, numVertices, indices.size, vertexAttributes );
	...
	private static KeyframedSubMesh readMesh( BufferedReader in, boolean flipV ) throws IOException {
		...
		KeyframedSubMesh mesh = new KeyframedSubMesh( name, new Mesh( VertexDataType.VertexArray, false, numVertices, indices.size, createVertexAttributes( hasNormals,
				uvSets.size ) ), buildVertices( numVertices, hasNormals, uvSets ), animations, animatedComponents, GL10.GL_TRIANGLES );
