...
public class Box2dLightTest implements ApplicationListener, InputProcessor {
	...
	public void create() {
		...
		for( int i = 0; i < BALLSNUM; i++ ) {
			final Color c = new Color( 0.05f, MathUtils.random(), MathUtils.random(), 1f );

			Light light = new PointLight( rayHandler, RAYS_PER_BALL, c, LIGHT_DISTANCE, 20, 10 );
	...
	public void render() {
		...
		for( int i = 0; i < BALLSNUM; i++ ) {
			...
			batch.draw( textureRegion, position.x - radius, position.y - radius, radius, radius, radius * 2, radius * 2, 1, 1,
					angle );
		}
		...
		rayHandler.setCombinedMatrix( camera.combined, camera.position.x, camera.position.y, camera.viewportWidth * camera.zoom,
				camera.viewportHeight * camera.zoom );
		...
		font.draw( batch, Integer.toString( Gdx.graphics.getFramesPerSecond() ) + " - light Rendered Last Frame: "
	...
	private void createPhysicsWorld() {
		...
		chainShape.createLoop( new Vector2[] { new Vector2( -22, 1 ), new Vector2( 22, 1 ), new Vector2( 22, 31 ),
				new Vector2( 0, 20 ), new Vector2( -22, 31 ), new Vector2( -22, 1 ) } );
